...
└── src/main/
    ...
    │   ├── Krypton.java          ~4300 Zeilen — die gesamte Client-Logik
    │   └── mixin/
    │       ├── CameraMixin.java
    │       ├── EntityMixin.java
    │       ├── ExampleMixin.java
    ...
    │       ├── InputAccessor.java
    │       ├── InteractionManagerAccessor.java
...
1. **Alle Settings laden** — `loadWhitelist()`, `loadKeybind()`, `loadFullbright()`,
   ...
   `loadSafetyLogout()`, `loadStaffDetect()`, `loadStaffGlyphs()`,
   `loadDisconnectLog()`.
...
und der echte Blickstrahl ihn weiterhin trifft. Der Gegnerstatus ist dafür
unerheblich.
...
| 4 | Frischen Klick erzeugen: `attackKey` drücken, Ziel raycasten und direkt `attackBlock()` aufrufen; das umgeht Vanillas `blockBreakingCooldown` wie `MinecraftClient.doAttack()` | — |
...
3. **Der Guard baut selbst ab.** State 4 startet den Vorgang nach einem echten
   Raycast mit `interactionManager.attackBlock()`; State 5 führt ihn über
   `updateBlockBreakingProgress()` + `swingHand()` fort. Der direkte Start
   entspricht Vanillas `MinecraftClient.doAttack()` und umgeht den fünf Ticks
   langen `blockBreakingCooldown` nach dem vorherigen Block. Damit hängt der Abbau an **keinem**
...
ohne Abbau-Paket. Vanillas 5 Ticks `blockBreakingCooldown` werden während dieser
Pause über `InteractionManagerAccessor` auf 0 zurückgesetzt; die Pause selbst
dauert damit **700–1100 ms**. Kürzer reicht auf einem
...
der nächste Abbau per direktem `attackBlock()` mit einem frischen
`START_DESTROY_BLOCK`, also einem sauber
getrennten Klick.
...
Auf der neuen Druckflanke läuft zusätzlich `KeyBinding.onKeyPressed()`, damit
auch `wasPressed()` einen frischen Klick sieht. `resetBlockBreaking()` ruft wie
Vanillas `handleBlockBreaking(false)` `cancelBlockBreaking()` auf und setzt den
Cooldown per Accessor zurück. Damit ist der Zustand an jeder Stelle konsistent,
die ihn abfragt.
...
vorübergehend unsichtbaren oder unerreichbaren Ziel unterscheiden.
...
| `KeyboardInputMixin` | `KeyboardInput` | `tick` @TAIL | Überschreibt in Freecam `input.playerInput` mit einem leeren `PlayerInput` **und** `input.movementVector` mit `(0,0)` (über `InputAccessor`) — seit 1.21.2 entsteht die Bewegung aus dem Vektor, nicht aus dem Record; ohne das zappelte der Körper mit WASD gegen die Wand → der Spielerkörper bewegt sich nicht mit. **Ausnahme Sneak:** läuft der Dauer-Sneak des Guards (`shouldForceSneak()`), bleibt die Sneak-Komponente `true` — der Körper duckt sich weiter, bewegt sich aber nicht. Komponenten-Reihenfolge von `PlayerInput`: `forward, backward, left, right, jump, sneak, sprint`. |
| `InputAccessor` | `Input` | `@Accessor("movementVector")` | Schreibzugriff auf das in der Oberklasse deklarierte `protected`-Feld. Ein `@Shadow` im `KeyboardInputMixin` kompilierte zwar, scheiterte aber beim Mixin-Apply, weil das Feld nicht in `KeyboardInput` selbst deklariert ist. |
| `InteractionManagerAccessor` | `ClientPlayerInteractionManager` | `@Accessor("blockBreakingCooldown")` | Setzt den Vanilla-Cooldown zwischen zwei gestackten Spawnern auf 0, damit der nächste direkte Klick ohne fünf tote Ticks beginnen kann. |
...
- `Krypton.java` ist mit ~4300 Zeilen ein Monolith aus `static`-State.
...
- **Neue Module** brauchen mindestens: ein `static boolean`-Flag, einen Eintrag
  ...
  `dotAnim` ist auf **24** Einträge dimensioniert; bei Index ≥ 24 muss das
